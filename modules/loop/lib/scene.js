 ...
 *    对象条目自动带 at="MM-DD HH:MM" 真实时间，供模型以时间为准判断新旧（2026-09-03-3 时间线加固）。
 *  - userInstructions：思维预设（2026-09-04-2）注入内容（内层 XML，null/空串则跳过）。 */
export function buildSceneBrief({ time, userContext, persona, memories, loopHistory, userInput, recentUserMessages, recentDialogue, memorySummaryBlock, userInstructions }) {
  ...
  if (userInput) blocks.push(`<user-input>${escapeXml(userInput)}</user-input>`); // 2026-08-30：对话前置思考携带用户最新输入
  // 2026-09-01：近期用户消息（感知用户状态的直接证据；时间驱动循环无 <user-input> 时也能判断状态变化）
  // 2026-09-03-2 时间线修复：每条消息带真实时间 at="MM-DD HH:MM"（取自记忆行 createdAt）——
  // 夜行作息下相邻两条用户消息可能相隔数小时/跨天（实机 08:57 与 23:59 相隔 15h），
  // 无时间戳会让模型把旧消息误判为"刚发生"（00:00:30"当然关心啊"串线推送的根因）。
  ...
  // 2026-09-03-3：最近真实对话实录（用户 ↔ AI 在对话中的回复，role 标注 + at 时间）——主动循环
  // 此前看不到"AI 刚在对话里说了什么"（记忆只记用户侧），导致主动发言与对话答复撞车/互相推翻
  // （实机 06:42 黄焖鸡 vs 鲅鱼水饺 割裂）。<recent-dialogue> 补齐该信息缺口。
  ...
  // 2026-09-04-2 思维预设：用户指令（<user-instructions>，紧跟人格之后、记忆之前注入——
  // 决策协议首步点名其为最高优先级，见 DECISION_PROTOCOL 第 0 步）
  if (userInstructions) blocks.push(`<user-instructions>\n${String(userInstructions)}\n</user-instructions>`);
  // 2026-09-03-9：双 Agent agent1 记忆概括（非空时替代原始 <memories> 注入，agent2 只读概括；
 ...
 *  2026-09-01 补充：显式加入"感知用户状态变化"步骤——自推理/思考中也须对照当前状态与
 *  近期用户消息判断状态变化，变化时必须在 actions 中决策 user_state_set/user_state_clear
 *  （与对话面的 user-state-maintenance 指令对齐，补上认知循环侧的状态感知缺口）。
 *  2026-09-03 记忆错乱修复：状态推断护栏——过去/将来时陈述与纯静默均不得作为
 *  当前状态推断依据（实机曾把 08:59 的"我去睡了"错位成 23:00、把 40 分钟静默推断成 active）。
 *  2026-09-03-2 时间线护栏：<recent-user-messages> 每条带真实 at 时间，判断新旧一律以 at 为准，
 *  不得假设相邻消息时间接近；<memories>/<loop-history> 中的 <loop-speak>/<loop-decision> 是 AI
 *  自身记录，不得当作用户发言（实机把 15h 前的提问当"一分钟前"、把 AI 问候当用户消息）。 */
