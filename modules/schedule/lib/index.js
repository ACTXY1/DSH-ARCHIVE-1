 ...
 *     "停机错过"等系统状态提示走 panel 级，只进通知流水不进对话）。
...
export function apply(ctx, rawConfig) {
  ...
  const api = {
    ...
    create(input) {
      ...
      // 非法 schedule 枚举直接抛错，不静默回退 one-time（防无 at 时下一 tick 立即触发）
      ...
      // one-time/daily 的 at 必须可解析（NaN/垃圾串会落成永不到期的幽灵任务）
    ...
    list(limit = 50) {
      // 钳制 limit：负数会使 SQLite LIMIT -1 返回全部任务
    ...
    cleanup() {
      ...
      // 纳入 failed：fire 抛错置 failed 的记录也需清理（否则成为永不清理的孤儿）
    ...
    close() {
      // 关闭时置位 disposed，防止检查 tick 在 db.close() 后触发
      // "database is not open" 未捕获异常
      disposed = true;
  ...
  function fire(task) {
    ...
    if (task.type === 'message') {
      try {
        ...
        // notify 限频会静默吞掉提醒（任务却标记 fired）→ 限频窗口内推迟重试、保持 pending，
        // 保证用户显式要求的提醒最终送达
        if (res && res.limited) {
    ...
    // 重复任务：重排下次
    // 重排须用 hydrateTask 输出的 camelCase 字段（intervalMinutes/dueAt）；误用 snake_case
    // 会得到 undefined → 重排到期=now → interval/daily 每 tick 反复重触发。
  ...
  ctx.timer.setInterval(() => {
    if (disposed) return; // 关闭后不再执行检查（防 db closed 未捕获异常）
    try {
      const now = Date.now();
 ...
 * 采用翻页遍历 + 精确匹配：高频对话记忆会把旧 task-due 挤出单页 200 窗口（保护残留），
 * 子串匹配会误伤不同任务（"提醒喝水"与"提醒喝水并吃药"互相解除），故按标准格式精确匹配。 */
