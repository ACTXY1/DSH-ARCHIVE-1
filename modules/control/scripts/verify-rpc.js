...
// apply 会立即触发 autoBackupCheck（写真实备份目录）。切换到临时 cwd，
// 使 DATA_ROOT/BACKUP_ROOT 落在 tmp 下（自动备份/导入测试的备份目录也由本脚本自建，无项目污染）。
// PROJECT_ROOT = dataRoot 上级的上级：为保持 PROJECT_ROOT=tmp 的语义，
// dataRoot 必须传 tmp/dsh/data（模拟真实层级 dsh/data），不能再用 cwd/data 兜底。
...
// 内存态 settings（models.setProviders 密钥保留合并逻辑的守卫依赖它）
...
const ctx = {
  ...
  get: (name) => {
    ...
    if (name === 'settings') return {
      // 内存态 settings：恒空对象无法验证 models.setProviders 的密钥保留合并逻辑
      register: () => ({}),
    ...
    if (name === 'directoryPicker') return { capability: () => ({ kind: 'native', pick: async (signal) => {
      // 守卫：native pick 契约要求 AbortSignal（pick 内部会读 signal.aborted），
...
// setProviders 守卫：先存密钥，再以"空 apiKey 整表回写"（模拟
// 启停/删除/编辑留空），断言密钥不被清空——空 apiKey 整表回写不得清空已存密钥。
...
// 一键更新：stub 环境 PROJECT_ROOT=tmp 非 git 仓库 → check 离线兜底；apply 缺脚本报错
