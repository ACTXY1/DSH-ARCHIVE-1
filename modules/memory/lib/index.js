 ...
 * 原因：execute 返回完整数据行（id/source/createdAt/…），而 output schema 声明
 * additionalProperties:false 且未列出这些字段 → agent 运行时校验拒绝（INVALID_TOOL_OUTPUT）。
 */
function pickFields(obj, keys) {
  // 过滤 null 与 undefined：可选字段（detail/evidence 等）未传时存储层返回 null，
  // 而 schema 声明 string → INVALID_TOOL_OUTPUT（如 user_state_set 未传 detail）。
  const out = {};
...
export function apply(ctx, rawConfig) {
  ...
  const api = {
    ...
    // 供人格一致性校验器（dsh-archive-consistency）复用同源 ollama embedding：
    // 一致性模块用同一模型/端点提取"人格状态"向量（零重复配置）。
  ...
  // 对话记忆自动落库：把每条真实用户输入写入记忆（source='conversation'，kind='episodic'，tag conversation），跨会话可召回。
  ...
  function contentTextOf(data) {
    ...
    // 纯字符串形态的 content 也要解析（与 loop 模块 contentTextOf 行为一致）
  ...
  /**
   * 用户状态维护：由 systemPrompt 注入的 user-state-maintenance 指令要求 AI 每回合
   * 读取注入的当前状态 → 结合本条用户消息隐式推断 → 仅在状态变化时调用
   * user_state_set/clear 修正（user_state_set 工具已带证据/TTL 参数，可完整维护）。
   * 此处仅保留对话记忆落库，零 LLM 成本。
   */
  ...
  // 注入后附 user-state-maintenance 指令：要求 AI 读取已注入状态后，在输出对话前隐式推断
  // 用户状态，变化时用 user_state_set/clear 修正。
  try {
  ...
  if (tools !== undefined) {
    ...
    reg(defineTool({
      ...
      execute(args) {
        // 仅状态名集合实质变化才 emit（同值状态刷新 TTL 不触发，避免无效调用驱动循环浪费 token）；
        ...
        // 状态变化事件发射：仅工具入口发射（agent/用户外部动作）；loop 内部动作走服务层不发射，
        // 避免"设状态→触发循环"自激。
    ...
    reg(defineTool({
      ...
      execute(args) {
        ...
        // 状态变化事件发射（同 user_state_set，仅工具入口）；仅实际清除（removed=true）才发射
        // （清一个不存在的状态不触发循环）
  ...
  // in-flight 守卫：手动按钮与每日自动定时器可能并发，防重复整合/重复软遗忘。
  ...
  api.integrate = async ({ windowMs = 24 * 3600000 } = {}) => {
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        // aborted 同样判失败（与 loop/subconscious/consistency 一致）：否则中止后半截内容
        // 会被当成最终摘要写入并软遗忘全部原条目（不可逆损伤）
  ...
  // 自动整合（每日最多一次；6h 检查一次）
  // 启动立即首检一次：窗口内 loop thought ≥12 才真正执行，不足则 skipped（无 token 浪费）；此后保持每日最多一次。
