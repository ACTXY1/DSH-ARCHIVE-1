 ...
 *
 * 需求：基于虚拟时钟的周期性自循环思考——事件触发 + 定时兜底（默认 5 分钟，可运行时调整）；
 ...
 *  - 用户主动发消息后立即暂停时间驱动循环（fallback/startup/hour）quietAfterUserMs（可配置），
 *    窗口内只运行对话循环（pre-turn）；若有正在运行的时间驱动循环/过期 pre-turn 循环则立刻中止，
 *    并清理其循环记录、回退其行为（不发言/不行动/不留档/不更新 lastDecision）；
 *    pre-turn 按 reason 无条件禁言（不依赖调用方传 suppressSpeak），超时后的后台收尾循环也被中止——
 *    防止两个并行思维循环基于同一时刻记忆做出并列而非延续的决策，割裂对话。
 *
 * 边界（诚实声明）：本插件决定"是否发言/是否行动"，发言与工具行动的实际执行
 * 通道由 agent 应用面/消息通道承载——决策经
...
const DEFAULTS = {
  ...
  // 单次决策调用最大 token 预算（默认 10000，总控「思维循环」页可运行时调整）。
  // deepseek-v4-flash 是推理模型，reasoning 会占用大量预算；预算过低时复杂简报易致正文截断，
  // 循环因"LLM 未返回文本"而失败。10000 预留充足思考空间。
  maxTokens: 10000,
  ...
  // 对话让路窗口：用户主动发消息/对话回合结束后暂停时间驱动循环的时长，窗口内只运行对话循环
  // （pre-turn），防并行决策割裂对话。防串线/割裂的治本（<recent-dialogue> 上下文回馈 + 记忆/注入
  // 带 at 时间线 + pre-turn 禁言 + action 门控）已独立就位，本窗口仅作最后兜底。
  quietAfterUserMs: 60000,
  // pre-turn（对话前置思考）专用：完成时限 + 单次预算（只产简短决策 JSON）。
  // 双 Agent 开启后 pre-turn 含"记忆概括+决策"两段 LLM 调用，时限须能覆盖两段；
  // 避免推理模型 reasoning 拖满预算超时。
  preTurnCapMs: 120000,
  preTurnMaxTokens: 4000,
  // 双 Agent 记忆加工 + 输出审查（默认关 = 与旧流程完全一致，总控可开）。
  //  - dualAgent=true：agent1 概括记忆（recent/语义两类严格分源）→ agent2 决策读概括而非原文；
  //    agent2 输出前审查（通顺/行动合理/不重复/符合人设/符合用户指令 → ok/revise/cancel）；对话回复经注入自检约束。
  ...
  // 睡眠/清醒期：用户"睡眠中"状态持续 ≥30 分钟且未来窗口内无定时任务 → 进入睡眠期
  // （循环完全暂停，无视降频开关）。睡眠期仅"真实唤醒信号"解除并进入 3 小时强制清醒间隔：
  // 用户状态被清除/改标为其他状态、用户主动发消息、定时任务打断。状态行 TTL 到期 ≠ 醒来
  // （夜行作息长睡眠可远超 TTL，属常态；到期后保持静默等真实唤醒信号，杜绝睡眠期连环打扰）；
  // sleepMaxMs 仅作为"无 TTL 睡眠行（直到手动清除）"的绝对兜底上限，防止状态卡死。
...
// 双 Agent 的记忆分析师（agent1）：严格分源概括；输出即 XML 数据块，供 agent2 决策读取
...
// 输出审查员：对拟发言/拟行动做多维度检查（通顺/行动合理/不重复/符合人设/符合用户指令）
// → 输出 ok/revise/cancel + 更正内容/阻止行动；「符合用户指令」在材料含【用户指令】时核验，无指令则通过。
...
// 对话回复正文的软自检约束（dualAgent 开启时注入主会话 systemPrompt；回复生成前自我核对）
...
function normalizeConfig(raw = {}) {
  ...
  // pre-turn（对话前置思考）专用参数：独立完成时限与单次预算（范围约束与 UI 编辑框一致，防乱填，见 PRE_TURN_* 常量注释）。
  ...
  // 双 Agent 开关与预算（预算范围护栏，防乱填致概括/审查输出不可用）
...
export function apply(ctx, rawConfig) {
  ...
  // 思维预设：内存态（默认空=零注入零变化）；settings 可用时回填并持久化
  let instr = defaultInstrState();
  let instrPersist = null;
  // archive-loop 命名空间由本模块注册（注册若晚于读取，降频开关重启后丢失）；register 后回填 saved 值。
  ctx.inject(['settings'], (settingsCtx) => {
    try {
      ...
      if (saved && typeof saved.dualAgent === 'boolean') config.dualAgent = saved.dualAgent;
      // 思维预设命名空间（本模块注册 + 回填 + 持久化引用）
  ...
  const state = {
    ...
    errorCount: 0,
    conversingSince: 0,
    // 用户消息静默窗口：quietUntil 之前暂停时间驱动循环（fallback/startup/hour），
    // 只运行对话（pre-turn）循环；activeCycle 记录进行中的循环，供用户消息到达时中止
    // （两个并行循环基于同一时刻记忆 → 行为并列而非延续 → 须中止并清理记录/回退行为）。
    quietUntil: 0,
    activeCycle: null,
    cancelledCount: 0,
    // 睡眠/清醒期：awake | asleep | cooldown（内存态，重启后从 awake 重新积累，
    ...
    // 每 30s 重试只会空转耗资源。进入冷却后时间驱动循环暂停低频探测，配置写入事件
    // （archive/credentials-changed）会立即唤醒重试。
   ...
   *  @param {{capAbort?:boolean}} [opts] capAbort=true 表示"对话前置思考上限"中止
   ...
   *  用户发消息即"真实唤醒信号"——若正处于睡眠期立即解除（睡眠期常因 TTL 到期而
   *  状态行已消失但人仍在睡，用户消息是唯一可靠的醒来判定）。 */
   ...
   *  排除 interval 周期任务——"每 N 分钟"任务每次触发都重排到窗口内，若计为"工作任务"则存在任一
   *  周期任务时智能体永不入睡（潜意识梦境引擎永不运行）；周期任务到点仍会经 wake('task') 打断睡眠。 */
   ...
   *    状态行 TTL 到期 ≠ 醒来——夜行作息长睡眠远超 TTL 是常态，到期后保持静默，
   *    等真实唤醒信号，杜绝睡眠期连环主动打扰。
   ...
   *  wake('user') 由 handleUserMessage 调用（用户发消息=真实唤醒信号）。 */
  ...
  function sleepSummary(now = Date.now()) {
    ...
    return {
      ...
      skips: s.skips,
      sleepExpirySeen: s.sleepExpirySeen, // 当前睡眠行到期点（诊断用）
  ...
  /** 通用 LLM 文本调用：自定义 OpenAI 兼容 provider 优先 → 失败回退官方；
  ...
  async function callText({ system, user, signal, maxTokens }) {
    ...
    if (custom) {
      try {
        ...
        // 兜底：正文为空但 reasoning_content 含输出（推理模型偶发把答案写进思考段）
  ...
  /** 召回记忆：按来源返回（供双 Agent 严格分源与单 Agent 拼装共用）。
  ...
  /** agent1 记忆分析师（双 Agent 开启时）：把召回记忆按 recent/语义 两类严格概括，
  ...
  /** 输出前审查（双 Agent 开启且确有输出时）：五维检查（通顺/行动合理/不重复/符合人设/
   *  符合用户指令）→ ok/revise/cancel + blockedActions。失败返回 {ok:false}（调用方跳过审查按原样投递，不阻断）。
   *  instructionsText = 本次注入的用户指令（可为空）。 */
  ...
  /** 近期用户消息（感知用户状态的直接证据）：取最近写入的对话记忆
   *  （source='conversation'，内容形如"用户：…"）去前缀后回传；上限 3 条防 token 膨胀。
   *  时间驱动循环（fallback/startup/hour）无 <user-input>，靠此块感知用户状态变化。
   *  取 200 条再过滤 conversation，保证跨数小时仍能找到用户消息（循环决策记忆每几分钟即写
   *  若干条，窗口过小会把用户消息挤出）。返回 {text, at}——带记忆行真实时间（createdAt）：
   *  用户消息可相隔很久，纯文本相邻排列会让模型误判为连续对话，带时间供模型判断新旧。 */
  ...
  // ── 最近真实对话缓冲（主会话 user↔ai；对话记忆只落"用户：…"，AI 回复仅在会话事件流，
  ...
  const INTERNAL_ACTIONS = {
    // 用户状态读写走 memory 服务的 state 命名空间（state.set/state.clear）
    // confidence 透传（与 user_state_set 工具一致，LLM 可给出置信度）
    ...
    // notify/schedule 经 ctx.get 可选访问（schedule 亦注入 loop → 显式 inject 会循环依赖死锁）
    // notify_send 内容由 AI 撰写、面向用户 → scope='chat'（受对话/静默窗门控，见 cycle）
    notify_send: (args) => {
  ...
  // 静默/免流水动作集：纯系统内自包含（用户状态维护/记忆笔记）或内容已自行送达（notify_send，
   ...
   *  必须 await，否则 rejection 变 unhandled → Node 进程崩溃。
   *  notify_send（AI 撰写、面向用户的 chat 级消息）在 pre-turn 对话循环或静默窗内不投递
   *  （对话中由 agent 正常回复承担，避免并列消息割裂对话）——此时动作标记 suppressed 不执行。 */
  ...
  async function cycle(reason, extra = {}) {
    // 对话占位（conversing）中允许跑 pre-turn——连续对话时若 turn/end 未及时复位占位，
    // 前置思考也不会被整段跳过（用户消息驱动的思维对每回合可见）。
    if (state.mode !== 'idle' && !(reason === 'pre-turn' && state.mode === 'conversing')) {
      return { skipped: true, reason: `busy:${state.mode}` };
    }
    // 凭证缺失冷却：未配置 API key 时循环必失败，30s 风暴只会空转耗资源。
    ...
    // 静默窗口内暂停时间驱动循环（fallback/startup/hour），只跑对话循环（pre-turn）
    if (isTimeBasedReason(reason) && Date.now() < state.quietUntil) return { skipped: true, reason: 'quiet' };
    // 睡眠期：先推进睡眠状态机；睡眠期中循环完全暂停（无视降频开关，事件触发也跳过）
    updateSleepState();
    ...
    state.mode = 'thinking';
    // pre-turn 不得清空 dirty——对话占位（conversing）中运行、用户连发消息会频繁触发 pre-turn cycle，
    // 若清 dirty 会把对话期间到达的 state-changed 等排队事件吞掉（状态变化将延迟到下次兜底才处理）；
    // dirty 只由时间/事件驱动循环消费。pre-turn 自身产生的 dirty 条目（trigger('pre-turn') 注入）
    // 单独移除，避免残留误导后续触发。
    if (reason === 'pre-turn') {
    ...
    // 登记进行中的循环，供用户消息到达时中止（activeCycle 同时携带 userCancelled 标记）
    state.activeCycle = { ac, reason, at: Date.now() };
    // pre-turn 的 LLM 上限取 max(thinkTimeoutMs, preTurnCapMs+10s)（让前置思考有正常完成时间，不被 thinkTimeout 先掐）；
    ...
    try {
      ...
      // 双 Agent（agent1）：记忆概括替代原始 <memories>（严格分源）；失败降级用原文
      ...
      // 近期用户消息作为状态感知证据注入简报（时间驱动循环无 <user-input> 的补充）；
      // recentDialogue=最近真实对话（含 AI 对话回复）——主动循环据此判断"是否已答复/延续对话"。
      // 思维预设：命中文本 = 用户最新输入 + 用户状态渲染 + 近期用户消息；命中后组装注入块。
      ...
      // 决策 JSON 解析失败（LLM 偶发输出非 JSON）重试一次并附格式纠正提示——避免循环因解析失败而空转
      let decision = null;
      let parseError = null;
      for (let attempt = 0; attempt < 2; attempt++) {
        // pre-turn 只产简短决策 JSON，用独立小预算（preTurnMaxTokens）加速完成；
      ...
      // 决策产出瞬间被用户消息中止 → 整轮作废（不发言/不行动/不留档/不更新 lastDecision）
      ...
      // 任何 pre-turn 发言都是并列输出，会割裂对话（由 reason 强制禁言，不依赖调用方传 suppressSpeak）。
      ...
      if (decision.shouldSpeak && !extra.suppressSpeak && reason !== 'pre-turn') {
        ...
        // 静默窗（用户消息/回合结束 quietAfterUserMs 内）对**一切**主动发言生效，不限于时间驱动循环
        // ——state-changed 等事件驱动循环也可能在回合结束后的窗口内触发并推送。窗口内不投递发言，
        // 决策与循环日志照常留档（同 pre-turn 语义）。
        ...
        // 双 Agent：输出前审查（仅对将实际投递的发言；可更正内容或取消本次发言）
        // 思维预设：审查材料附本次注入的用户指令（核对第⑤维）
      ...
      if (decision.shouldAct && decision.actions.length > 0) {
        ...
        //    （发言已禁；行动状态串/notify_send 若投递会插进对话中间）。
        ...
        if (!hasAssessment) {
          ...
          if (decision.actionsAllowed) {
            // 双 Agent：输出前审查——阻止不合理的行动（不新增行动，仅过滤）
            // 思维预设：审查材料附本次注入的用户指令（核对第⑤维）
        ...
        // 行动执行期间被用户消息中止 → 不再告知（防并列输出的割裂感）
        if (state.activeCycle?.userCancelled === true) return { skipped: false, cancelled: true, reason };
        // 告知（panel 级操作流水）：由 AI 依据后果评估判定（notifyUser）——小事可静默行动。
        // 告知规则：
        //  1) pre-turn（对话中）不产生任何告知——对话内动作保持静默；
        ...
        if (decision.notifyUser && !isPreTurnCycle) {
          ...
          if (parts.length > 0) {
            // notify 经 ctx.get 可选访问（显式 inject 会与 schedule 构成循环依赖）
            const notify = ctx.get('notify');
            if (notify !== undefined) {
              try {
      ...
      state.cycleCount++;
      // 事件监听者异常不判循环失败重跑（监听者抛错不计失败、不置 dirty，避免整轮循环重复执行 LLM/动作/发言）
      ...
      // 用户消息中止 → 非故障：不计数错误、不置 dirty（避免下个 tick 用旧记忆重跑并行决策）
      if (state.activeCycle?.userCancelled === true) {
        state.cancelledCount++;
        // 对话前置思考上限（cap）中止 → 补一条"超时"留痕记录，让「思维循环」活动流可见该回合曾有
        // 思考被砍（避免被砍的思考完全静默消失）；不计错误、不置 dirty。
      ...
      state.errorCount++;
      // 凭证缺失冷却：MISSING_CREDENTIAL（未配置 API key）是"等用户配置"的持久性错误，
      // 30s 风暴只会空转耗资源 → 进入 5 分钟冷却，冷却期内 tick 全部跳过；模型页配置成功会
      // emit archive/credentials-changed 立即唤醒。
      ...
      if (isCredentialMissing && state.credentialCooldownUntil <= Date.now()) {
        ...
        // 循环失败直写 stdout（进 archive.log）——dsh logger 输出不落 archive.log，
        // 仅靠 logger 时失败期间的具体错误码不可见。
        bootLine(`[archive-loop] 循环失败(${reason}) error#${state.errorCount}: ${error.message}`);
      }
      // 失败保留 dirty 标志——断网/API 瞬时故障后下一个 tick（30s）自动重试，
  ...
  /** 最近 1 次循环的决策摘要（供简报 <loop-history>）。带真实时间 at，防旧决策被当"刚才"。 */
  ...
  /** 主动发言交付（经 notify 通道送达控制台/通知流水/总会话；同时事件 + 记忆留档）。
   *  AI 面向用户的自然语言 → scope='chat'（可注入总会话对话流，同 <loop-speak> 留档）；
   *  "🤖 主动行动"等操作状态串一律 panel 级不注入对话（见 cycle 告知分支）。
   *  @param {{cancelled?:boolean}} [opts] 被用户消息中止的循环不再写 <loop-speak> 留档 */
  function emitSpeak(content, time, { cancelled } = {}) {
    ...
    if (cancelled) return; // 并行决策的发言不留档（清理记录/回退行为）
    // memory.write 异步（含 embedding），必须 .catch 兜底，
    // 否则掉线/超时时 unhandled rejection → Node 崩溃。
  ...
  /** 当前生效的兜底间隔（降频开关开启则用降频间隔，不再依赖用户状态）。 */
   ...
   *  静默窗口内暂停时间驱动触发（fallback/startup/hour），对话循环优先。
   *  凭证冷却期内不触发时间驱动循环（避免 30s 风暴空转；cycle 入口兜底跳过）。 */
  ...
  // 凭证配置成功唤醒：模型页写入 DeepSeek key / 自定义 provider 后清除冷却并立即重试
  ...
  // 时钟里程碑：整点变化
  const tick = () => {
    // 睡眠状态推进（进入/解除/冷却倒计时；即使无循环触发也要及时更新）
    updateSleepState();
  ...
  const api = {
    ...
    /** 运行时调整（无需重启；支持降频开关，经 settings archive-loop 持久化）。 */
    configure: (patch = {}) => {
      ...
      // 睡眠参数运行时调整（UI/测试用；默认值在 cordis.patch.yml）
      ...
      // 静默窗口运行时调整（≥0 可关闭该限制；默认值在 cordis.patch.yml）
      ...
      // pre-turn 专用参数运行时调整（范围与 UI 一致，防乱填）
      ...
      // 双 Agent 开关运行时调整（持久化 archive-loop settings；默认关=与旧流程一致）
      ...
      return { fallbackIntervalMs: config.fallbackIntervalMs, reducedMode: config.reducedMode, effectiveFallbackMs: effectiveFallback(), quietAfterUserMs: config.quietAfterUserMs, maxTokens: config.maxTokens, preTurnCapMs: config.preTurnCapMs, preTurnMaxTokens: config.preTurnMaxTokens, dualAgent: config.dualAgent };
    },
    /** 对话前置：若思考循环进行中，有界等待其完成；随后运行一次 pre-turn 决策循环并进入对话占位（暂停主动循环）。
     * 每条对话消息都应产生一次围绕该消息的前置思考（决策携带 <user-input>，循环日志落库），不做节流。
     *  @param {string} [userInput] 用户最新输入（供 pre-turn 决策围绕该输入分析） */
    beforeTurn: async (userInput) => {
      // 对话开始前兜底中止仍在运行的时间驱动/过期 pre-turn 循环（session/event 已先行中止；
      ...
      // 对话占位（conversing）中也跑 pre-turn——连续对话时若 turn/end 未及时复位占位，
      // 前置思考也不会被整段跳过；conversing 只拦 tick 的主动循环。
    ...
    state: () => {
      ...
      return {
        ...rest,
        activeCycleReason: state.activeCycle?.reason ?? null,
        quietUntil: state.quietUntil, // 静默窗口截止时间
        quietLeftMs: Math.max(0, state.quietUntil - now), // 静默窗口剩余
        cancelledCount: state.cancelledCount, // 被用户消息中止的循环数
    ...
    stats: () => ({
      ...
      config: {
        ...
        quietAfterUserMs: config.quietAfterUserMs,
        maxTokens: config.maxTokens,
        preTurnCapMs: config.preTurnCapMs, // 对话前置思考完成时限（ms）
        preTurnMaxTokens: config.preTurnMaxTokens, // 对话前置思考单次预算
    ...
    /** 定时任务打断睡眠期（schedule 触发任务时调用）：asleep → cooldown。 */
    wake,
    /** 思维预设：用户指令管理。get=全量读取；save=整存（校验+护栏+持久化）；
  ...
  // 思维预设持久化（settings 不可用/失败时静默降级为内存态，不影响功能）
  ...
  try {
    ctx.systemPrompt.context({
      ...
      text: () => {
        if (!state.lastDecision) return '';
        // 附带 reason/at——对话回合据此判断注入的是"本轮前置思考"还是旧的时间驱动决策
  ...
  // 对话回复正文的输出前自检（仅 dualAgent 开启时注入——回复正文由主会话 agent 生成，
  // 不经 cycle 决策，故用"生成前软自检"约束：通顺/承接不重复不推翻/行动合理/符合人设）。
  // 思维预设：scope 含 dialogue 的常驻指令附加在自检后（运行时读取当前预设，无需重启）
  try {
    ctx.systemPrompt.context({
  ...
  // 对话前置思考接线：主会话 agent 每回合第一步（step===1）在 LLM 请求前运行 beforeTurn
  // （pre-turn 决策循环：召回记忆→场景简报→LLM 决策→防护→循环日志 thought），决策注入
  // loop-latest-analysis 供本回合参考；回合结束 endConversation 恢复 idle。
  // 仅主会话（session-main）触发——子代理/其他会话不跑，防 token 浪费。pre-step 为瀑布事件，
  // 必须 next() 放行，失败不阻断对话。
  ...
  // 用户最新输入双保险：pre-step 的 messages 提取 + session/event user/message 缓冲兜底
  // （部分回合从 messages 提取不到用户输入，pre-turn 决策不能写成"无新输入"）。
  ...
  ctx.on('agent/pre-step', async ({ agent, step, messages }, next) => {
    try {
      if (step === 1 && isMainAgent(agent)) {
        ...
        // 兜底：messages 提取为空但 60s 内收到过真实用户消息 → 用缓冲（避免 pre-turn"无新输入"）
        ...
        // 前置思考设独立上限（preTurnCapMs）——断网/API 挂起时不让对话回合被卡到 thinkTimeoutMs；
        // 上限先到则对话照常进行，且不放任后台自收尾（见下方 abortParallelCycle）。
        let capTimer = null;
        const cap = new Promise((resolve) => { capTimer = setTimeout(() => resolve('cap'), config.preTurnCapMs); });
        // .finally 保证 preTurn 成功/失败都清理 capTimer（失败路径不得遗留悬空定时器）
        ...
        // 上限（cap）先到 → 对话已推进，立即中止仍在后台收尾的 pre-turn 循环。
        // 其决策基于旧输入（"并列"记忆），若放任收尾会：更新 lastDecision 污染下一回合注入、
        // 写留档记录，且可能投递主动发言。
        // capAbort=true → cycle 补"超时留痕"（活动流可见被砍的思考，见 cycle 中止分支）。
  ...
  ctx.on('session/event', (session, event) => {
    try {
      // 用户消息缓冲（pre-turn userInput 兜底；与 memory 落库同源事件）；仅主会话的用户消息
      // 进入缓冲（其他会话消息混入会让主会话 pre-turn 的 <user-input> 用错输入）
      ...
      if (event?.type === 'user/message' && event?.data?.source?.kind === 'user' && isMain) {
        ...
        if (txt && !txt.startsWith('Current runtime context')) {
          ...
          pushDialogue('user', txt, Date.now()); // 对话实录（user 侧）
        }
      }
      // AI 在对话中的最终回复 → 对话实录（ai 侧；主动循环据此看到对话答复，见 <recent-dialogue>）
      ...
      if (event?.type === 'turn/end' && isMain) {
        api.endConversation();
        // 对话回合结束也刷新静默窗口（主动发言不得紧贴刚结束的对话发言）。只在用户消息设置的
