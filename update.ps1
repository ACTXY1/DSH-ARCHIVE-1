...
try {
    ...
    if (Test-Path $presetSrc) {
        $presetDst = Join-Path $env:USERPROFILE '.dsh\.agent-presets\archive-standard'
        ...
        if (Test-Path $presetDst) { Write-Ok 'agent preset 已同步' }
        else { Write-Warn 'agent preset 同步失败（~/.dsh/.agent-presets），更新继续。' }
    }
