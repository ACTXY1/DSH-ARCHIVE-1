 ...
 *  -  限制：用户主动发消息后立即暂停时间驱动循环（fallback/startup/hour）quietAfterUserMs
 ...
 * 通道由 agent 应用面/消息通道（对话面、主动消息）承载——决策经
...
const DEFAULTS = {
  ...
  //：思维循环 LLM 总上限 60s→120s（用户指示"延长思维循环默认时限到 120 秒"）——
  ...
  //：用户主动发消息后暂停时间驱动循环的时长（窗口内只运行对话循环，防并行决策割裂对话）。
  //  记忆错乱修复：30s→180s——对话回合结束后 60s 内仍可能触发 proactive 重复/串线发言
  // （实机  00:00:30 在用户消息 67s 后推送了一条 15h 前旧问答的串线回复）。
  // /6 修正（遵用户指示撤回）：本字段=对话让路窗口，原值 30s；曾为防 00:00:30 串线提至 180s、
  ...
  // /9：对话前置思考（pre-turn）专用——完成时限 120s + 单次预算 4000（只产简短决策 JSON）。
  // 120s 为用户指示：双 Agent 开启后 pre-turn 含"记忆概括+决策"两段 LLM，时限须覆盖两段；
  // 放宽后也减少"思考被砍只剩留痕"。避免推理模型 reasoning 拖满预算超时。
  preTurnCapMs: 120000,
  preTurnMaxTokens: 4000,
  //：冷启动首条免等（"挂机后首条消息迟迟不回显"修复）——
  ...
  //   对话中的连续消息仍走 preTurnCapMs（默认 120s，遵用户  指示放宽）。
  ...
  // 睡眠/清醒期（ 用户设计）：用户"睡眠中"状态持续 ≥30 分钟且未来窗口内无
  // 定时任务 → 进入睡眠期（循环完全暂停，无视降频开关）；任务触发/用户状态清除或改标签
  // /用户发消息 → 解除并进入 3 小时强制清醒间隔（期间无法再睡眠）。
  //  记忆错乱修复（语义修正）：状态 TTL 到期 ≠ 醒来——睡眠期不再因行过期或
...
//：冷启动判定阈值与临时上限的范围（normalizeConfig/configure 双端一致）
...
//：双 Agent —— agent1 记忆分析师（严格分源概括；输出即 XML 数据块，供 agent2 决策读）
...
//：输出审查员（对拟发言/拟行动做四维检查并给出更正/取消/阻止）
//  思维预设：增第⑤维「符合用户指令」（材料含【用户指令】；无指令则本项通过）
...
function normalizeConfig(raw = {}) {
  ...
  //：对话前置思考（pre-turn）专用参数——推理模型 reasoning 常超 25s/高预算，
  // 此前 cap 25s 固定、预算共用 maxTokens(10000)，pre-turn 频繁被砍只剩留痕（用户实测反馈）。
  //：范围约束（5s~10min / 500~60000）与 UI 编辑框一致，防乱填（见 PRE_TURN_* 常量注释）。
  ...
  //：冷启动首条免等参数（范围护栏与 configure 一致；coldCap 再与 preTurnCapMs 取小）
...
export function apply(ctx, rawConfig) {
  ...
  //  （major）：archive-loop 命名空间由本模块注册（此前在 control 注册，
   ...
   *  @param {{capAbort?:boolean}} [opts]：capAbort=true 表示"25s 对话前置思考上限"中止
   ...
   *   记忆错乱修复：用户发消息即真实唤醒信号——若正处于睡眠期立即解除
   ...
   *   （设计冲突）：排除 interval 周期任务——"每 N 分钟"任务每次触发都重排到
   ...
   *    状态行 TTL 到期 ≠ 醒来（ 修复：夜行长睡眠远超 TTL 是常态，
  ...
  /** 通用 LLM 文本调用（ 重构）：自定义 OpenAI 兼容 provider 优先 → 失败回退官方；
  ...
  /** 召回记忆（ 起按来源返回，供双 Agent 严格分源与单 Agent 拼装共用）。
   ...
   *   思维预设：instructionsText = 本次注入的用户指令（可为空）。 */
   ...
   *   实机修复：提取窗口从 20 扩大到 200（store.list 上限）——循环决策记忆
   ...
   *   时间线修复：返回 {text, at}——带上记忆行真实时间（createdAt）。
  ...
  const INTERNAL_ACTIONS = {
    // 用户状态读写走 memory 服务的 state 命名空间（ 修复：此前调不存在的 stateSet/stateClear 顶层方法，动作永远 failed）
    ...
    //  分级：notify_send 内容由 AI 撰写、面向用户 → scope='chat'（受对话/静默窗门控，见 cycle）
    notify_send: (args) => {
   ...
   *  必须 await，否则 rejection 变 unhandled → Node 进程崩溃（ fatal load failure 根因）。
   *   门控：notify_send（AI 撰写、面向用户的 chat 级消息）在 pre-turn 对话循环或静默窗内
  ...
  async function cycle(reason, extra = {}) {
    //：对话占位（conversing）中允许跑 pre-turn——此前仅 idle 可跑，若 turn/end 未及时
    ...
    state.mode = 'thinking';
    //  连锁修复：pre-turn（现允许对话占位 conversing 中运行）不得清空 dirty——
    ...
    //：pre-turn 的 LLM 上限放宽到 preTurnCapMs+10s（让前置思考有正常完成时间，不被 60s thinkTimeout 先掐）；
    ...
    try {
      ...
      //：recentDialogue = 最近真实对话（含 AI 对话回复）——主动循环判断"是否已答复/延续对话"
      //  思维预设：命中文本 = 用户最新输入 + 用户状态渲染 + 近期用户消息；组装注入块
      ...
      // 任何 pre-turn 发言都是并列输出，会割裂对话。 修复：由 reason 强制（此前依赖调用方传
      ...
      if (decision.shouldSpeak && !extra.suppressSpeak && reason !== 'pre-turn') {
        ...
        //  时间线/重复推送修复：静默窗（用户消息/回合结束 quietAfterUserMs 内）对
      ...
      if (state.activeCycle?.userCancelled === true) {
        ...
        //：25s 对话前置思考上限（cap）中止 → 补一条"超时"留痕记录，让「思维循环」活动流
   ...
   *   分级：AI 面向用户的自然语言 → scope='chat'（可注入总会话对话流，同 <loop-speak> 留档）；
  ...
  /** 当前生效的兜底间隔（ 降频开关制：开启则用降频间隔，不再依赖用户状态）。 */
  ...
  const api = {
    ...
    configure: (patch = {}) => {
      ...
      //  静默窗口运行时调整（≥0 可关闭该限制；默认 30000 在 cordis.patch.yml）
      ...
      // /5 对话前置思考专用参数运行时调整（默认 90s/4000；范围与 UI 一致，防乱填）
      ...
      //：冷启动首条免等参数运行时调整（默认 15min/5s；范围与 normalizeConfig 一致）
     ...
     *  修复：去掉 preTurnRefreshMs(60s) 节流——用户明确要求"对话过程进入认知循环"，
  ...
  ctx.on('agent/pre-step', async ({ agent, step, messages }, next) => {
    try {
      if (step === 1 && isMainAgent(agent)) {
        ...
        //  兜底：messages 提取为空但 60s 内收到过真实用户消息 → 用缓冲（修复 pre-turn"无新输入"）
        ...
        //：冷启动首条免等——距上一真实用户回合超过 preTurnColdGapMs（默认 15min）时，
        ...
        // 仍走 preTurnCapMs（默认 120s，遵用户  指示放宽）。
        const now = Date.now();
        ...
        //  韧性补丁：前置思考设等待上限——断网/API 挂起时不让对话回合被卡住；
        // 上限先到则对话照常进行； 起不再放任后台自收尾（见下方 abortParallelCycle）。
        let capTimer = null;
        const cap = new Promise((resolve) => { capTimer = setTimeout(() => resolve('cap'), capMs); });
        //  .finally 保证 preTurn 成功/失败都清理 capTimer（此前失败路径遗留悬空定时器）
  ...
  ctx.on('session/event', (session, event) => {
    try {
      //：用户消息缓冲（pre-turn userInput 兜底；与 memory 落库同源事件）
      ...
      //：AI 在对话中的最终回复 → 对话实录（ai 侧；主动循环此前完全看不到对话答复，见 <recent-dialogue>）
      ...
      if (event?.type === 'turn/end' && isMain) {
        api.endConversation();
        //  记忆错乱修复：对话回合结束也刷新静默窗口（proactive 不得紧贴刚结束的
