...
#  起链接固定在各自项目 home 内，天然互不干扰；此处仍做一致性校验。
...
#  fix: verify the 11434 listener is really an ollama before reusing it -
...
if [ "$ready" -eq 1 ]; then
  #：新版 dsh web（≥0.1.2）带浏览器鉴权，需打开 dsh 打印的带 token 地址
